...
MP3_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../context/new-service-test.mp3"))

@pytest.fixture
def mock_db():
    with patch("ainulindale_api.apps.eridian_echo.api.models.create_job", new_callable=AsyncMock) as mock_create, \
         patch("ainulindale_api.apps.eridian_echo.api.models.get_job", new_callable=AsyncMock) as mock_get, \
         patch("ainulindale_api.apps.eridian_echo.api.models.get_jobs_for_owner", new_callable=AsyncMock) as mock_get_owner, \
         patch("ainulindale_api.apps.eridian_echo.service.models.update_job_status", new_callable=AsyncMock) as mock_update:
         
        # Create a fake job
        fake_job = models.Job(id="test-123", owner_id="owner-456", filename="new-service-test.mp3")
...
def test_eridian_echo_e2e_gemini_transcription(mock_db) -> None:
    ...
    with open(MP3_PATH, "rb") as f:
        # Note: TestClient doesn't run background tasks by default unless we wait for them, 
